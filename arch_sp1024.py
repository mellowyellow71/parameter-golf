...
from kernels import fused_softcap_cross_entropy as _fused_ce_kernel, tuned_linear as _tuned_linear
import torch.nn.functional as _F_ref

# Set False if Triton GEMM is slower than cuBLAS on your GPU (check step times)
_USE_TRITON_GEMM = True

def fused_softcap_cross_entropy(logits, targets, softcap):
    return _fused_ce_kernel(logits, targets, softcap)

def tuned_linear(x, w):
    return _tuned_linear(x, w) if _USE_TRITON_GEMM else _F_ref.linear(x, w.to(x.dtype))
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(
        ...
        return tuned_linear(y, out_w), raw_v
...
class MLP(nn.Module):
    ...
    def forward(self, x: Tensor, up_w: Tensor, down_w: Tensor) -> Tensor:
        x = F.leaky_relu(tuned_linear(x, up_w), negative_slope=0.5)
        return tuned_linear(x.square(), down_w)
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        main_loss = fused_softcap_cross_entropy(logits_proj, targets, self.logit_softcap)
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            ...
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                mtp_loss_sum = mtp_loss_sum + fused_softcap_cross_entropy(
