...
def flash_attn_3_func(q: "Tensor", k: "Tensor", v: "Tensor", causal: bool = False) -> "Tensor":
    # q: (B, T, Hq, D)  k/v: (B, T, Hkv, D) — transpose to (B, H, T, D) for SDPA
    q = q.transpose(1, 2)
    k = k.transpose(1, 2)
    v = v.transpose(1, 2)
...
class Hyperparameters:
    ...
    gptq_n_batches = int(os.environ.get("GPTQ_N_BATCHES", 32))
...
def eval_val(
    ...
    if local_batch_tokens < seq_len:
        raise ValueError(
            f"VAL_BATCH_SIZE too small: {args.val_batch_size}, WS={world_size}, "
            f"GA={grad_accum_steps}, seq_len={seq_len}"
        )
...
class GPT(nn.Module):
    def __init__(
        ...
        self.ve_layer_indices = (
            [int(x) for x in ve_layers.split(",") if x.strip()] if ve_enabled else []
        )
...
def quantize_int6_per_row(t: Tensor, cr: int = 31) -> tuple[Tensor, Tensor]:
    t32 = t.float()
    if t32.ndim == 2:
        ...
        for pct in [0.9990, 0.9995, 0.9999, 0.99999, 1.0]:
            ...
            q = torch.clamp(torch.round(t32 / s.float()[:, None]), -cr, cr).to(
                torch.int8
            )
    ...
    s = torch.tensor(amax / cr if amax > 0 else 1.0, dtype=torch.float16)
...
def _rebank_state_dict(
    sd: dict[str, Tensor], num_layers: int, tpl: dict[str, Tensor]
) -> dict[str, Tensor]:
    out: dict[str, Tensor] = {}
    n = num_layers
    qo = [None] * (2 * n)
    ...
    for i in range(n):
        for k, lst, off in [
            ...
            (f"blocks.{i}.mlp.proj.weight", dn, i),
        ]:
            if k in sd:
                lst[off] = sd[k]
                consumed.add(k)
    out["qo_bank"] = torch.stack(qo).to(dtype=tpl["qo_bank"].dtype)
    out["kv_bank"] = torch.stack(kv).to(dtype=tpl["kv_bank"].dtype)
    out["mlp_up_bank"] = torch.stack(up).to(dtype=tpl["mlp_up_bank"].dtype)
    out["mlp_down_bank"] = torch.stack(dn).to(dtype=tpl["mlp_down_bank"].dtype)
    for name, tensor in sd.items():
...
def gptq_quantize_weight(
    W: Tensor, H: Tensor, nb: int = 6, bs: int = 128, damp: float = 0.01
) -> tuple[Tensor, Tensor]:
    rows, cols = W.shape
    W = W.float().clone()
    H = H.float().clone()
    mv = 2 ** (nb - 1) - 1
    dm = torch.mean(H.diagonal()).item()
    if dm < 1e-10:
        rm = W.abs().amax(dim=1)
        sc = (rm / mv).clamp_min(1.0 / mv).to(torch.float16)
        qi = torch.clamp(torch.round(W / sc.float()[:, None]), -mv - 1, mv).to(
            torch.int8
        )
        return qi, sc
    d = damp * dm
    ...
    try:
        ...
        H.diagonal().add_(d * 10)
        try:
            L = torch.linalg.cholesky(H)
            Hi = torch.cholesky_inverse(L)
        except torch.linalg.LinAlgError:
            Hi = torch.linalg.pinv(H)
    # Find optimal per-row scales
    if W.ndim == 2:
        bsc, berr = None, float("inf")
        for pct in [0.9990, 0.9995, 0.9999, 0.99999, 1.0]:
            rc = (
                torch.quantile(W.abs(), pct, dim=1)
                if pct < 1.0
                else W.abs().amax(dim=1)
            )
            s = (rc / mv).clamp_min(1.0 / mv).to(torch.float16)
            q = torch.clamp(torch.round(W / s.float()[:, None]), -mv - 1, mv)
            err = (W - q * s.float()[:, None]).pow(2).mean().item()
            if err < berr:
                bsc, berr = s, err
        scales = bsc
    else:
        rm = W.abs().amax(dim=1)
        scales = (rm / mv).clamp_min(1.0 / mv).to(torch.float16)
    sf = scales.float()
    Q = torch.zeros(rows, cols, dtype=torch.int8, device=W.device)
    for cs in range(0, cols, bs):
        ce = min(cs + bs, cols)
        bc = ce - cs
        Wb = W[:, cs:ce].clone()
        ...
        for j in range(bc):
            w = Wb[:, j]
            dd = Hib[j, j].clamp_min(1e-10)
            qi = torch.clamp(torch.round(w / sf), -mv - 1, mv)
            qd = qi * sf
            Q[:, cs + j] = qi.to(torch.int8)
            e = (w - qd) / dd
            Eb[:, j] = e
            if j + 1 < bc:
                Wb[:, j + 1 :] -= e.unsqueeze(1) * Hib[j, j + 1 :].unsqueeze(0)
        if ce < cols:
            W[:, ce:] -= Eb @ Hi[cs:ce, ce:]
    return Q, scales
...
def collect_layer_hessians(
    ...
    nl = model.num_layers
    n = nl
    nenc = model.num_encoder_layers
    ndec = model.num_decoder_layers
    hess: dict[str, Tensor] = {}

    def _acc(key: str, H: Tensor) -> None:
        if key in hess:
            hess[key].add_(H)
        else:
            hess[key] = H.clone()
    ...
    def _layer(
        i: int, x: Tensor, x0: Tensor, xb: Tensor, vc: dict, v0: Tensor | None, bsz: int
    ) -> tuple[Tensor, Tensor | None]:
        block = model.blocks[i]
        attn = block.attn
        mix = block.resid_mix.to(dtype=x.dtype)
        xi = mix[0][None, None, :] * x + mix[1][None, None, :] * x0
        ai = block.attn_norm(xi) * block.ln_scale_factor
        af = ai.reshape(-1, ai.shape[-1]).float()
        with torch.autocast(device_type="cuda", enabled=False):
            Ha = af.T @ af
        _acc(f"qo_bank.{i}", Ha)
        _acc(f"kv_bank.{i}", Ha.clone())
        _acc(f"kv_bank.{n + i}", Ha.clone())
        qw = model.qo_bank[i]
        kw = model.kv_bank[i]
        vw = model.kv_bank[n + i]
        ow = model.qo_bank[n + i]
        q = F.linear(ai, qw.to(ai.dtype)).reshape(
            bsz, seq_len, attn.num_heads, attn.head_dim
        )
        k = F.linear(ai, kw.to(ai.dtype)).reshape(
            bsz, seq_len, attn.num_kv_heads, attn.head_dim
        )
        v = F.linear(ai, vw.to(ai.dtype))
        ve = model._get_ve(i, xb, vc)
        if ve is not None:
            v = v + ve
        v = v.reshape(bsz, seq_len, attn.num_kv_heads, attn.head_dim)
        rv = v if attn.value_residual else None
        if attn.value_residual and v0 is not None:
            lam = attn.vr_lambda.to(dtype=v.dtype)
            v = lam[0] * v0 + lam[1] * v
        q = F.rms_norm(q, (q.size(-1),))
        k = F.rms_norm(k, (k.size(-1),))
        cos, sin = attn.rotary(seq_len, x.device, q.dtype)
        ...
        if attn.gated_attention:
            gv = torch.sigmoid(attn.attn_gate(ai)).unsqueeze(-1)
            y = y * gv
        oi = y.reshape(bsz, seq_len, -1)
        of = oi.reshape(-1, oi.shape[-1]).float()
        with torch.autocast(device_type="cuda", enabled=False):
            _acc(f"qo_bank.{n + i}", of.T @ of)
        ao = F.linear(oi, ow.to(oi.dtype))
        if v0 is None and rv is not None:
            v0 = rv
        xo = xi + block.attn_scale.to(dtype=xi.dtype)[None, None, :] * ao
        mi = block.mlp_norm(xo) * block.ln_scale_factor
        mf = mi.reshape(-1, mi.shape[-1]).float()
        with torch.autocast(device_type="cuda", enabled=False):
            _acc(f"mlp_up_bank.{i}", mf.T @ mf)
        uw = model.mlp_up_bank[i]
        dw = model.mlp_down_bank[i]
        mu = F.leaky_relu(F.linear(mi, uw.to(mi.dtype)), negative_slope=0.5)
        md = mu.square()
        df = md.reshape(-1, md.shape[-1]).float()
        with torch.autocast(device_type="cuda", enabled=False):
            _acc(f"mlp_down_bank.{i}", df.T @ df)
        mo = F.linear(md, dw.to(md.dtype))
        xo = xo + block.mlp_scale.to(dtype=xo.dtype)[None, None, :] * mo
        if block.dtg_gate is not None:
            gv = torch.sigmoid(block.dtg_gate(xi.detach()))
            xo = xi + gv * (xo - xi)
        return xo, v0

    total_tokens = val_tokens.numel() - 1
    usable_seqs = total_tokens // seq_len
    batch_seqs = max(1, min(8, usable_seqs // max(n_batches, 1)))
    with torch.no_grad(), torch.autocast(device_type="cuda", dtype=torch.bfloat16):
        for bi in range(n_batches):
            ss = (bi * batch_seqs) % usable_seqs
            st = ss * seq_len
            et = st + batch_seqs * seq_len + 1
            ...
            vc: dict = {}
            for i in range(nenc):
                x, v0 = _layer(i, x, x0, xb, vc, v0, bsz)
                skips.append(x)
            for di in range(ndec):
                bi2 = nenc + di
                if skips:
                    x = (
                        x
                        + model.skip_weights[di].to(dtype=x.dtype)[None, None, :]
                        * skips.pop()
                    )
                x, _ = _layer(bi2, x, x0, xb, vc, v0, bsz)
    for key in hess:
        hess[key] = hess[key].cpu()
    model.train()
    return hess
...
def gptq_quantize_model(
...
) -> tuple[dict[str, Tensor], dict[str, object], int, int]:
    nl = model.num_layers
    n = nl
    sd_cpu = {
        k: v.detach().cpu()
    ...
    usd = _unbank_state_dict(sd_cpu, nl)
    result: dict[str, Tensor] = {}
    meta: dict[str, object] = {}
    w2h: dict[str, str] = {}
    for i in range(n):
        w2h[f"blocks.{i}.attn.c_q.weight"] = f"qo_bank.{i}"
        w2h[f"blocks.{i}.attn.proj.weight"] = f"qo_bank.{n + i}"
        w2h[f"blocks.{i}.attn.c_k.weight"] = f"kv_bank.{i}"
        w2h[f"blocks.{i}.attn.c_v.weight"] = f"kv_bank.{n + i}"
        w2h[f"blocks.{i}.mlp.fc.weight"] = f"mlp_up_bank.{i}"
        w2h[f"blocks.{i}.mlp.proj.weight"] = f"mlp_down_bank.{i}"
    gc = 0
    fc = 0
    for name, tensor in usd.items():
        ...
        hk = w2h.get(name)
        if hk is not None and hk in hessians and t.ndim == 2:
            H = hessians[hk].cpu().float()
            qi, s = gptq_quantize_weight(t, H, nb=n_bits, bs=blocksize, damp=dampening)
            result[name + ".q"] = qi
            result[name + ".scale"] = s
            meta[name] = {"type": "int6"}
            gc += 1
            ...
            result[name + ".scale"] = s
            meta[name] = {"type": "int8"}
    return result, meta, gc, fc


def _update_sens_ema(model: nn.Module, ema: dict[str, float], decay: float = 0.99) -> None:
    """Update EMA of per-slice squared gradient norms for the 4 weight banks."""
    nl = model.num_layers
    n = nl
    banks = [
        ("qo_bank", model.qo_bank, 2 * n),
        ("kv_bank", model.kv_bank, 2 * n),
        ("mlp_up_bank", model.mlp_up_bank, n),
        ("mlp_down_bank", model.mlp_down_bank, n),
    ]
    for bank_name, bank, count in banks:
        if bank.grad is None:
            continue
        g = bank.grad.detach()
        for i in range(count):
            key = f"{bank_name}.{i}"
            g2 = g[i].float().pow(2).mean().item()
            ema[key] = decay * ema.get(key, g2) + (1.0 - decay) * g2
...
def solve_bit_allocation(
    ...
    """
    LP: maximize sum(sensitivity_i * bits_i)
    subject to: sum(n_params_i * bits_i / 8) <= budget_bytes
                bits_min <= bits_i <= bits_max

    Sensitive layers receive more bits; returns key -> rounded int bits.
    """
    nl = model.num_layers
    n = nl
    banks = [
    ...
    for bank_name, bank, count in banks:
        for i in range(count):
            key = f"{bank_name}.{i}"
            keys.append(key)
            sens_vals.append(float(sensitivities.get(key, 1.0)))
            param_counts.append(float(bank[i].numel()))

    m = len(keys)
    if m == 0:
        return {}

    s = np.array(sens_vals, dtype=np.float64)
    s_max = s.max()
    if s_max > 0:
        s = s / s_max
    p = np.array(param_counts, dtype=np.float64)

    b = cp.Variable(m)
    prob = cp.Problem(
        cp.Maximize(s @ b),
        [b >= bits_min, b <= bits_max, (p / 8.0) @ b <= float(budget_bytes)],
    )
    try:
    ...
    if b.value is not None:
        for key, bv in zip(keys, b.value):
            alloc[key] = float(np.clip(round(float(bv)), int(bits_min), int(bits_max)))
    else:
        for key in keys:
            alloc[key] = 6.0
    return alloc
...
def _quantize_intN_per_row(t: Tensor, nb: int) -> tuple[Tensor, Tensor]:
    """Per-row symmetric quantization to nb bits, stored as int8."""
    mv = (1 << (nb - 1)) - 1
    t32 = t.float()
    if t32.ndim == 2:
        rc = t32.abs().amax(dim=1)
        s = (rc / mv).clamp_min(1.0 / mv).to(torch.float16)
        q = torch.clamp(torch.round(t32 / s.float()[:, None]), -mv - 1, mv).to(torch.int8)
        return q, s
    amax = t32.abs().max().item()
    s = torch.tensor(amax / mv if amax > 0 else 1.0, dtype=torch.float16)
    q = torch.clamp(torch.round(t32 / s.float()), -mv - 1, mv).to(torch.int8)
    return q, s
...
def apply_bit_allocation(
    ...
    """GPTQ quantization with per-layer bit widths from LP solution."""
    nl = model.num_layers
    n = nl
    sd_cpu = {k: v.detach().cpu() for k, v in model.state_dict().items() if "mtp_heads" not in k}
    usd = _unbank_state_dict(sd_cpu, nl)
    result: dict[str, Tensor] = {}
    meta: dict[str, object] = {}
    w2h: dict[str, str] = {}
    ...
    for name, tensor in usd.items():
        ...
        if any(p in name for p in CONTROL_TENSOR_NAME_PATTERNS):
            ...
            continue
        hk = w2h.get(name)
        nb = int(bit_alloc.get(hk, 6)) if hk is not None else 6
        cat = _classify_param(name)
        if hk is not None and hk in hessians and t.ndim == 2:
            H = hessians[hk].cpu().float()
            qi, s = gptq_quantize_weight(t, H, nb=nb, bs=blocksize, damp=dampening)
            result[name + ".q"] = qi
            result[name + ".scale"] = s
            meta[name] = {"type": f"int{nb}"}
            gc += 1
        elif cat in {"mlp", "attn"} and t.ndim >= 1:
            q, s = _quantize_intN_per_row(t, nb)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": f"int{nb}"}
            fc += 1
...
def main() -> None:
    ...
    if base_model.ve_shared is not None:
        tok_params.append(
            {
                "params": [base_model.ve_shared.embed.weight],
                "lr": token_lr,
                "base_lr": token_lr,
            }
        )
    ...
    sens_ema: dict[str, float] = {}
    current_bit_alloc: dict[str, float] = {}
    ...
    while True:
        ...
        if should_validate:
            ...
            if sens_ema:
                current_bit_alloc = solve_bit_allocation(base_model, sens_ema)
                if current_bit_alloc:
                    alloc_vals = list(current_bit_alloc.values())
                    log0(
                        f"bit_alloc: min={min(alloc_vals):.0f} max={max(alloc_vals):.0f} "
                        f"mean={sum(alloc_vals)/len(alloc_vals):.2f} n={len(alloc_vals)}"
                    )
            torch.cuda.synchronize()
            t0 = time.perf_counter()
        if last_step:
        ...
        if optimizer_head is not None:
            optimizer_head.step()
        optimizer_muon.step()
        _update_sens_ema(base_model, sens_ema)
        zero_grad_all()
        with torch.no_grad():
    ...
    if args.lawa_enabled and len(lawa_queue) > 1:
        ...
        avg_state = {
            name: torch.zeros(t.shape, dtype=torch.float32, device="cpu")
    ...
    if args.gptq_enabled:
        log0(f"gptq:collecting Hessians n_batches={args.gptq_n_batches}")
        torch.cuda.synchronize()
        ...
        torch.cuda.synchronize()
        log0(
            f"gptq:hessians_collected n_hessians={len(hessians)} "
            f"time={1000.0 * (time.perf_counter() - t_hess):.0f}ms"
        )
        t_quant = time.perf_counter()
        if current_bit_alloc:
            log0(
                f"gptq:using LP bit allocation n_weights={len(current_bit_alloc)} "
                f"bits_min={min(current_bit_alloc.values()):.0f} "
                f"bits_max={max(current_bit_alloc.values()):.0f} "
                f"bits_mean={sum(current_bit_alloc.values())/len(current_bit_alloc):.2f}"
            )
            quant_result, quant_meta, gptq_cnt, fallback_cnt = apply_bit_allocation(
                base_model,
                hessians,
                current_bit_alloc,
            ...
            quant_result, quant_meta, gptq_cnt, fallback_cnt = gptq_quantize_model(
                base_model,
                hessians,
                n_bits=6,
                blocksize=args.gptq_blocksize,
        ...
        torch.cuda.synchronize()
        log0(
            f"gptq:quantized gptq_weights={gptq_cnt} fallback_weights={fallback_cnt} "
            f"time={1000.0 * (time.perf_counter() - t_quant):.0f}ms"
        )
        del hessians
    else:
    ...
    if args.eval_stride > 0 and args.eval_stride < sw_seq_len:
        ...
        log0(
            f"final_int8_zlib_roundtrip_exact val_loss:{sw_val_loss:.8f} val_bpb:{sw_val_bpb:.8f}"
        )
    if args.eval_stride > 0 and args.eval_stride != 64 and 64 < sw_seq_len:
        ...
        log0(
            f"final_int8_zlib_roundtrip_exact val_loss:{sw64_val_loss:.8f} val_bpb:{sw64_val_bpb:.8f}"
        )
    if args.ttt_enabled:
        torch.cuda.synchronize()
